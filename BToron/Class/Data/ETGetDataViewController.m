//
//  ETGetDataViewController.m
//  BToron
//
//  Created by EndoTsuyoshi on 2014/06/11.
//  Copyright (c) 2014年 tuyo.en. All rights reserved.
//
#import "BSDefaultViewObject.h"
#import "ETGetDataViewController.h"

@interface ETGetDataViewController ()

@end

@implementation ETGetDataViewController


+ (instancetype)sharedClient
{
    static ETGetDataViewController *_sharedClient = nil;
    ...
    dispatch_once(&onceToken, ^{
        ...
        _sharedClient = [[ETGetDataViewController alloc]
...
//           [BSUserManager sharedManager].sessionId =
//           [responseObject valueForKeyPath:@"result.Auth.session_id"];
//           
...
- (void)getJsonDataWithKey:(NSString *)api_key
         completion:(void (^)(NSDictionary *, NSError *))block
{
    //message	◯	"""発送済みの商品について購入者様にメールを送信します。この処理は戻すことができません。ステータスを「キャンセル」にしてもよろしいでしょうか？”"
    //    error
    //    message	◯	”クレジットカード決済のキャンセルはできません。”	エラーの場合のみ必須
    //https://coinotron.com/coinotron/AccountServlet?action=api&api_key=B76B4056B7CD42B191D3FBFFBBAC810F
    NSString *strApi = @"api";
    //?action=api&api_key=B76B4056B7CD42B191D3FBFFBBAC810F
    //[self GET:@"/orders/get_changing_status_message"
    [self POST:@"/coinotron/AccountServlet"
   parameters:@{
                @"action"   : strApi,
                @"api_key"     : api_key,
                }
